...
function IconPhone({ className = "size-3.5" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6">
...
function IconMail({ className = "size-3.5" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6">
...
function Item({ icon, children, href, className = "" }: ItemProps) {
  ...
  const cls = `inline-flex items-center gap-2 hover:text-[var(--bone)] transition-colors ${className}`;
  if (href) {
    return (
...
export function TopUtilityBar({ variant = "dark" }: { variant?: "dark" | "light" }) {
  const textCls =
    variant === "dark"
      ? "text-[12px] text-[var(--bone)]/70"
      : "text-[12px] text-[var(--ink)]/55";

  return (
    <div className={`h-9 flex items-center justify-end gap-3 sm:gap-4 ${textCls}`}>
